...
const setCurrentPrize = () => {
  for (let i = 0; i < prizeConfig.prizeConfig.prizeList.length; i++) {
    if (prizeConfig.prizeConfig.prizeList[i].id == luckyID.value) {
      prizeConfig.prizeConfig.currentPrize = prizeConfig.prizeConfig.prizeList[i]
    }
  }
}
const luckyTargets = ref<any[]>([])
const luckyCardList = ref<number[]>([])
let luckyCount = ref(1)
let luckyID = ref('')
...
onMounted(() => {
  if (currentPrize.value) {
    luckyID.value = String(currentPrize.value.id)
  }
  initTableData();
  init();
  animation();
        ...
        <div id="menu">
            ...
            <div v-if="currentStatus == 1">
              ...
              <div class="mb-4 flex items-center justify-center">
                <select class="w-full max-w-xs select select-warning select-sm mr-4" v-model="luckyID" @change="setCurrentPrize">
